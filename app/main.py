...
from app.config import REQUEST_TIMEOUT_SECONDS, OPENAI_API_KEY
from app.graph import InterpretationError, cache_source, get_graph, run_interpretation
...
async def lifespan(app):
    # Local initialization only: no paid calls, cached answers or test data.
    get_graph()
    if OPENAI_API_KEY:
        get_client()
    yield
...
app = FastAPI(title="GridWise LLM-Assisted Energy Optimizer", lifespan=lifespan)
